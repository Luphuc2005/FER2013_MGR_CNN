...
export MGR_TF_INTRA_OP_THREADS="${MGR_TF_INTRA_OP_THREADS:-2}"
export MGR_TF_INTER_OP_THREADS="${MGR_TF_INTER_OP_THREADS:-2}"
export MGR_TF_DATA_NUM_PARALLEL_CALLS="${MGR_TF_DATA_NUM_PARALLEL_CALLS:-2}"
export MGR_TF_DATA_PRIVATE_THREADPOOL_SIZE="${MGR_TF_DATA_PRIVATE_THREADPOOL_SIZE:-2}"
export MGR_TF_DATA_DETERMINISTIC="${MGR_TF_DATA_DETERMINISTIC:-0}"
export MGR_PREFETCH_BUFFER="${MGR_PREFETCH_BUFFER:-2}"
export MGR_SHUFFLE_BUFFER="${MGR_SHUFFLE_BUFFER:-28709}"
...
RESUME_FLAG=()
if [ "${MGR_RESUME:-0}" = "1" ]; then
  RESUME_FLAG=(--resume)
fi

PRIMARY_BATCH="${MGR_PRIMARY_BATCH_SIZE_PER_GPU:-8}"
FALLBACK_BATCH="${MGR_FALLBACK_BATCH_SIZE_PER_GPU:-8}"
...
run_training() {
  ...
  "${PYTHON_BIN}" train.py --config "${CONFIG_PATH}" "${RESUME_FLAG[@]}" 2>&1 | tee -a "${TRAIN_LOG}"
}
