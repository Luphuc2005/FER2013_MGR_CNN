#!/usr/bin/env python3
"""Pre-flight validation suite for RAF-DB V5 Oversampled Optimal (PP1).

Verifies:
  1. Static configuration assertions:
     - Model: rafdb_v5_combined_ultimate_oversampled_optimal
     ...
     - Unfreeze: standard freeze_backbone_epochs=4, unfreeze at epoch 5
...
CONFIG_NAME = "config_rafdb_v5_combined_ultimate_oversampled_optimal.yaml"
...
def check_static_config(cfg_path: Path):
    ...
    assert m["name"] == "rafdb_v5_combined_ultimate_oversampled_optimal", f"Unexpected name {m['name']}"
...
def main():
    ...
    print(f"=== Validating RAF-DB V5 Oversampled Optimal (PP1): {cfg_path.name} ===")
