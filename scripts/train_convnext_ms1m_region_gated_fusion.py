"""Train ConvNeXt-B MS1M Multi-Scale Region Gated Fusion for FER2013.

This script executes the Multi-Scale Region Gated Fusion experiment, using a shared region dictionary (256-dim),
...
Writes strictly to outputs/tf_runs/convnext_base_ms1m_region_gated_fusion without modifying existing runs.
"""
...
from models.convnext_ms1m_region_gated_fusion import ConvNeXtMS1MRegionGatedFusionFER, count_params
from train import build_optimizer, configure_gpus, configure_tensorflow_runtime, resolve_phase_lrs, set_optimizer_lr


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ConvNeXt-B MS1M Multi-Scale Region Gated Fusion on FER2013.")
    parser.add_argument("--config", type=str, default="config_convnext_base_ms1m_region_gated_fusion.yaml")
...
def run_shape_smoke_test(model: ConvNeXtMS1MRegionGatedFusionFER, sample_inputs: Dict[str, tf.Tensor]) -> Dict[str, object]:
    print("=" * 72, flush=True)
...
def run_gradient_smoke_test(model: ConvNeXtMS1MRegionGatedFusionFER, sample_inputs: Dict[str, tf.Tensor], sample_labels: tf.Tensor, cfg: Dict) -> Dict[str, float]:
    print("=" * 72, flush=True)
...
def make_train_step(model: ConvNeXtMS1MRegionGatedFusionFER, cfg: Dict, optimizer_head, optimizer_backbone, train_backbone: bool):
...
def evaluate_dataset(model: ConvNeXtMS1MRegionGatedFusionFER, dataset: tf.data.Dataset, cfg: Dict, use_tta_hflip: bool = False) -> Dict[str, object]:
    y_true: List[int] = []
    y_pred: List[int] = []
    total_loss = 0.0
    total_count = 0
...
def main() -> int:
    ...
    with tf.device("/GPU:0" if tf.config.list_logical_devices("GPU") else "/CPU:0"):
        model = ConvNeXtMS1MRegionGatedFusionFER(cfg)
    ...
    print("=" * 72, flush=True)
    print("STARTING TRAINING: ConvNeXt-B MS1M Multi-Scale Region Gated Fusion", flush=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        print(
            ...
            f"gate_mean={row['gate_mean']:.4f} best_val_acc={best_acc:.4f}",
            flush=True,
        )
