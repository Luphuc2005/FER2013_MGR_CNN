"""Train ConvNeXt-B MS1M S3->S4 Late Cross Attention with Direct Feature Fusion.

This script executes the new experiment variant where S3->S4 cross attention
features are directly fused into the classifier head alongside the safe residual G4.

It writes strictly to outputs/convnext_ms1m_late_cross_fusion without modifying existing runs.
"""
...
from models.convnext_ms1m_late_cross_fusion import ConvNeXtMS1MLateCrossFeatureFusionFER, count_params
from train import build_optimizer, configure_gpus, configure_tensorflow_runtime, resolve_phase_lrs, set_optimizer_lr


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ConvNeXt-B MS1M Late Cross-Stage Fusion on FER2013.")
    parser.add_argument("--config", type=str, default="config_kaggle_convnext_ms1m_late_cross_fusion.yaml")
...
def run_shape_smoke_test(model: ConvNeXtMS1MLateCrossFeatureFusionFER, sample_inputs: Dict[str, tf.Tensor]) -> Dict[str, object]:
    print("=" * 72, flush=True)
...
def run_identity_smoke_test(model: ConvNeXtMS1MLateCrossFeatureFusionFER, sample_inputs: Dict[str, tf.Tensor], cfg: Dict) -> Dict[str, float]:
    print("=" * 72, flush=True)
    ...
    delta4_norm = float(outputs["delta4_norm"].numpy())
    g4_norm = float(outputs["g4_norm"].numpy())
    cross_ratio = float(outputs["cross_ratio"].numpy())
    ...
    print(f"  delta4_norm: {delta4_norm:.8e}", flush=True)
    print(f"  g4_norm: {g4_norm:.8e}", flush=True)
    print(f"  cross_feature_ratio: {cross_ratio:.8e}", flush=True)
...
def run_gradient_smoke_test(model: ConvNeXtMS1MLateCrossFeatureFusionFER, sample_inputs: Dict[str, tf.Tensor], sample_labels: tf.Tensor, cfg: Dict) -> Dict[str, float]:
    print("=" * 72, flush=True)
    ...
    with tf.GradientTape(persistent=True) as tape_dir:
        inputs_tensor = sample_inputs["image"] if isinstance(sample_inputs, dict) else sample_inputs
        endpoints_dir = model.rgb_baseline.backbone(inputs_tensor, training=True, return_endpoints=True)
        s3_dir = endpoints_dir["stage3"]
        s4_dir = endpoints_dir["stage4"]
        p3_dir = model.project_s3_to_s4(s3_dir, training=True)
        delta4_raw_dir = model.s3_s4_global(s4_dir, p3_dir, training=True)
        delta4_dir = tf.cast(delta4_raw_dir, tf.float32)
        tape_dir.watch(delta4_dir)
        f_cross_dir = tf.reduce_mean(delta4_dir, axis=[1, 2])
        p_cross_dir = model.proj_cross(f_cross_dir, training=True)
    grad_delta4_direct = tape_dir.gradient(p_cross_dir, delta4_dir)
    grad_delta4_direct_norm = float(tf.norm(grad_delta4_direct).numpy())
    del tape_dir

    if grad_delta4_direct_norm <= 0.0 or not math.isfinite(grad_delta4_direct_norm):
        raise RuntimeError(f"Direct fusion gradient smoke failed: grad_delta4_direct_norm={grad_delta4_direct_norm:.8e}")
    ...
    result = {
        ...
        "direct_branch_grad_norm_to_delta4": float(grad_delta4_direct_norm),
    }
...
def make_train_step(model: ConvNeXtMS1MLateCrossFeatureFusionFER, cfg: Dict, optimizer_head, optimizer_backbone, train_backbone: bool):
    ...
    def train_step(inputs, labels):
        ...
        return {
            ...
            "delta4_norm": tf.cast(outputs["delta4_norm"], tf.float32),
            "g4_norm": tf.cast(outputs["g4_norm"], tf.float32),
            "cross_ratio": tf.cast(outputs["cross_ratio"], tf.float32),
        }

    return train_step


def evaluate_dataset(model: ConvNeXtMS1MLateCrossFeatureFusionFER, dataset: tf.data.Dataset, cfg: Dict, use_tta_hflip: bool) -> Dict[str, object]:
    y_true: List[int] = []
    y_pred: List[int] = []
    total_loss = 0.0
    total_count = 0
    alpha4_values: List[float] = []
    delta4_norms: List[float] = []
    g4_norms: List[float] = []
    ...
    for inputs, labels in dataset:
        ...
        delta4_norms.append(float(outputs["delta4_norm"].numpy()))
        g4_norms.append(float(outputs["g4_norm"].numpy()))
        cross_ratios.append(float(outputs["cross_ratio"].numpy()))
    ...
    arr_a4 = np.asarray(alpha4_values, dtype=np.float64) if alpha4_values else np.asarray([0.0])
    metrics["alpha4_mean"] = float(np.mean(arr_a4))
    metrics["alpha4_min"] = float(np.min(arr_a4))
    metrics["alpha4_max"] = float(np.max(arr_a4))
    metrics["alpha4_std"] = float(np.std(arr_a4))
    metrics["delta4_norm"] = float(np.mean(delta4_norms)) if delta4_norms else 0.0
    metrics["g4_norm"] = float(np.mean(g4_norms)) if g4_norms else 0.0
    metrics["cross_ratio"] = float(np.mean(cross_ratios)) if cross_ratios else 0.0
...
def configure_gpus_safe(cfg: Dict) -> None:
    ...
    if os.environ.get("MGR_GPU_IDS") == "-1" or gpu_ids == [-1]:
        try:
            tf.config.set_visible_devices([], "GPU")
        except Exception:
            pass
        print("[INFO] CPU mode forced by configuration (MGR_GPU_IDS=-1 or gpu_ids=[-1]).", flush=True)
        return
    try:
        configure_gpus(cfg)
...
def main() -> int:
    ...
    with tf.device("/GPU:0" if tf.config.list_logical_devices("GPU") else "/CPU:0"):
        model = ConvNeXtMS1MLateCrossFeatureFusionFER(cfg)
    ...
    print("=" * 72, flush=True)
    print("STARTING TRAINING: ConvNeXt-B MS1M S3->S4 Late Cross Attention + Direct Feature Fusion", flush=True)
    print(f"SAM rho={float(cfg['training'].get('sam_rho', 0.03)):.4f} | mixed_precision=mixed_float16", flush=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for step, (inputs, labels) in enumerate(train_ds, start=1):
            ...
            batch_record = {k: float(metrics[k].numpy()) for k in (
                ...
                "delta4_norm", "g4_norm", "cross_ratio",
            )}
            ...
            if progress_interval and step % progress_interval == 0:
                print(
                    ...
                    f"grad_norm={grad_norm_v:.6f} alpha4={batch_record['alpha4_mean']:.4f} "
                    f"cross_ratio={batch_record['cross_ratio']:.4f}",
                    flush=True,
                )
        ...
        row = {
            ...
            "val_delta4_norm": float(val_metrics["delta4_norm"]),
            "val_g4_norm": float(val_metrics["g4_norm"]),
            "val_cross_ratio": float(val_metrics["cross_ratio"]),
        ...
        print(
            ...
            f"alpha4={row['alpha4_mean']:.4f} delta4_norm={row['val_delta4_norm']:.4f} "
            f"cross_ratio={row['val_cross_ratio']:.4f} best_val_acc={best_acc:.4f}",
            flush=True,
        )
