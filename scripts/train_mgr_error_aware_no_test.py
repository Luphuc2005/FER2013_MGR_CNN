...
from scripts.train_mgr_error_aware_weighted_ce import (  # noqa: E402
    anchor_weights,
    evaluate_val_oracle,
    make_train_step,
    metric_value,
...
from metrics.classification import save_metrics  # noqa: E402


def _build_split_dataset(cfg, split: str, *, training: bool):
    ...
    limit = cfg["data"].get(f"max_{split}_samples")
    if limit is not None:
        keep = min(int(limit), len(records.labels))
        records.images = records.images[:keep]
        records.labels = records.labels[:keep]
        records.sample_ids = records.sample_ids[:keep]
        if records.mask_paths is not None:
            records.mask_paths = records.mask_paths[:keep]
        if records.masks is not None:
            records.masks = records.masks[:keep]
    return make_dataset(records, cfg, split=split, training=training, replicas=1)
...
def main() -> int:
    ...
    if str(cfg["training"].get("loss")) != "error_aware_weighted_ce":
        raise RuntimeError("training.loss must be error_aware_weighted_ce for this entrypoint.")
    if float(cfg["training"].get("label_smoothing", 0.0)) != 0.0:
        raise RuntimeError("training.label_smoothing must be 0.0 for exact weighted CE.")
    if float(cfg["model"].get("ortho_loss_weight", 0.0)) != 0.0:
        raise RuntimeError("model.ortho_loss_weight must be 0.0 for this experiment.")
    if float(cfg["model"].get("cnn_aux_loss_weight", 0.0)) != 0.0:
        raise RuntimeError("model.cnn_aux_loss_weight must be 0.0 for this experiment.")
    ...
    print("[TRAIN_ENTRYPOINT] script=train_mgr_error_aware_no_test.py no_test=True", flush=True)
    print(
        f"[TRAIN_CONTRACT] loss={cfg['training'].get('loss')} "
        f"label_smoothing={float(cfg['training'].get('label_smoothing', 0.0)):.1f} "
        f"ortho_loss_weight={float(cfg['model'].get('ortho_loss_weight', 0.0)):.1f} "
        f"cnn_aux_loss_weight={float(cfg['model'].get('cnn_aux_loss_weight', 0.0)):.1f}",
        flush=True,
    )

    train_ds = _build_split_dataset(cfg, "train", training=True)
    val_ds = _build_split_dataset(cfg, "val", training=False)
    ...
    for epoch in range(int(ckpt_epoch.numpy()), int(cfg["training"]["epochs"])):
    ...
    if history:
        with (run_dir / "training_history.csv").open("w", encoding="utf-8") as f:
            f.write(",".join(history[0].keys()) + "\n")
            for row in history:
                f.write(",".join(str(row[k]) for k in history[0].keys()) + "\n")

    checksum_delta = abs(model_checksum(anchor_model) - anchor_checksum_before)
    print(f"[CNN_ANCHOR_UNCHANGED] checksum_delta={checksum_delta:.12f}", flush=True)
