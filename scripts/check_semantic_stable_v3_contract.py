...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate the RAF-DB semantic-stable v3 config contract.")
    parser.add_argument(
        "config",
        nargs="?",
...
def main() -> int:
    ...
    model_cfg = cfg["model"]
    clip_cfg = model_cfg["clip_semantic"]
    ...
    if str(model_cfg.get("name", "")) != "rafdb_siglip2_semantic_stable_v3":
        raise AssertionError("model.name must use the semantic-stable v3 lineage")
    if Path(cfg["paths"]["output_dir"]).name != "rafdb_siglip2_semantic_stable_v3":
        raise AssertionError("output_dir must use the independent semantic-stable v3 lineage")

    schedule_text = ", ".join(f"ep{epoch}={value:.2f}" for epoch, value in actual_schedule.items())
    print("SEMANTIC_STABLE_V3_CONFIG_OK")
