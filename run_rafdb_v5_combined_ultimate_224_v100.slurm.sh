...
CONFIG="$ROOT/config_rafdb_siglip2_semantic_stable_v5_combined_ultimate_224.yaml"
OUTPUT_DIR="$ROOT/outputs/papers/rafdb_siglip2_semantic_stable_v5_combined_ultimate_224"
...
# Optional smoke test before full run
"$FER_PY" -u scripts/smoketest_rafdb_v5_combined_ultimate_224.py --config "$CONFIG"

"$FER_PY" -u train.py --config "$CONFIG" --no-auto-increment
echo "Finished: $OUTPUT_DIR (v5 Combined Ultimate 224: Dynamic Part Attention + Adaptive Fusion Gate + Light Aug)."
