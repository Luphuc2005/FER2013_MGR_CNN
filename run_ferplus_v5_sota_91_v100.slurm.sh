#!/bin/bash
#SBATCH --job-name=FERPLUS_V5_SOTA
...
#SBATCH --mem=64G
#SBATCH --output=/home/ptbao/projects/FER2013_MGR_CNN/logs/FERPLUS_V5_SOTA_%j.out
#SBATCH --error=/home/ptbao/projects/FER2013_MGR_CNN/logs/FERPLUS_V5_SOTA_%j.err
set -euo pipefail

ROOT=/home/ptbao/projects/FER2013_MGR_CNN
cd "$ROOT"
FER_PY="$ROOT/fer2013_env/bin/python"
CONFIG="$ROOT/config_ferplus_v5_sota_91.yaml"
OUTPUT_DIR="$ROOT/outputs/papers/ferplus_v5_sota_91"
...
echo 'FERPlus V5 SOTA (Target >= 91.0%): Contempt (800) + Disgust (800) + Fear (1000) + Direct 1024-dim ArcFace Head'
echo 'Non-decaying Semantic Supervision (0.10 -> 0.15) + R-Drop Consistency (lambda_rdrop=0.30) + Targeted Hard Pairs'
echo 'Selection via val_accuracy (No-TTA). Target single-model test accuracy >= 91.0%.'

"$FER_PY" -u scripts/smoketest_ferplus_pipeline.py --num-batches 1 --config "$CONFIG"
if [[ "${SMOKE_ONLY:-0}" == "1" ]]; then
    echo 'FERPLUS_V5_SMOKE_ONLY_COMPLETE; training not started.'
    exit 0
fi
...
echo "FERPLUS_V5_TRAIN_COMPLETE: $OUTPUT_DIR"
