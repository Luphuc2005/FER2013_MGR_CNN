"""
Generate 20 publication-quality Grad-CAM candidate samples per emotion for FER2013 test set.
...
def _run_main():
    print("=" * 75)
    print(" FER2013: Generating Top 20 Grad-CAM Candidates Per Emotion (Publication Quality)")
    ...
    print("\n[1/5] Building AMGSA-FER model and restoring ckpt-29...")
    ...
    print(f"\n[2/5] Loading FER2013 test set from {csv_path}...")
    df_test = pd.read_csv(csv_path)
    print(f"  -> Total test samples: {len(df_test)}")

    # 4. Smart Candidate Selection: Pick 40-55 diverse candidate samples per emotion
    print("\n[3/5] Selecting and evaluating candidate test samples...")
    # For Disgust (emotion 1), evaluate all 55 samples.
    # For others, sample 45 diverse indices across the test set to ensure 20+ correct high-conf.
    candidate_indices = []
    for c in range(7):
        c_indices = df_test[df_test["emotion"] == c].index.tolist()
        if c == 1:
            candidate_indices.extend(c_indices)  # all 55
        else:
            # take 45 evenly spaced indices
            step = max(1, len(c_indices) // 45)
            selected_c = c_indices[::step][:45]
            candidate_indices.extend(selected_c)
            
    df_eval = df_test.loc[candidate_indices].copy()
    print(f"  -> Total candidates to evaluate: {len(df_eval)} samples across 7 emotions")
    ...
    confidences = []
    batch_size = 64
    eval_rows = [row for _, row in df_eval.iterrows()]
    ...
    for i in range(0, len(eval_rows), batch_size):
        ...
        for r in batch:
            ...
            im = Image.fromarray(pix, mode="L").resize((112, 112), resample=Image.Resampling.BILINEAR).convert("RGB")
            arr = (np.array(im, dtype=np.float32) / 255.0 - mean) / std
            batch_imgs.append(arr)
        
        batch_tensor = tf.convert_to_tensor(np.stack(batch_imgs, axis=0), dtype=tf.float32)
        out = model({"image": batch_tensor}, training=False)
        ...
        print(f"  Processed {min(i + batch_size, len(eval_rows))}/{len(eval_rows)} candidates...")

    df_eval["dataset_idx"] = candidate_indices
    df_eval["true_label"] = df_eval["emotion"].astype(int)
    df_eval["pred_label"] = pred_labels
    df_eval["confidence"] = confidences
    df_eval["correct"] = df_eval["true_label"] == df_eval["pred_label"]

    # 5. Filter top 20 correct samples with highest confidence for each emotion
    print("\n[4/5] Filtering Top 20 Best Samples per Emotion...")
    N_CANDIDATES = 20
    top20_by_emotion: Dict[int, pd.DataFrame] = {}

    for c in range(7):
        emo_name = EMOTION_NAMES_TITLE[c]
        c_correct = df_eval[(df_eval["true_label"] == c) & (df_eval["correct"])].copy()
        c_correct = c_correct.sort_values(by="confidence", ascending=False)
        
        if len(c_correct) >= N_CANDIDATES:
            top20 = c_correct.iloc[:N_CANDIDATES]
        else:
            rem = N_CANDIDATES - len(c_correct)
            c_other = df_eval[(df_eval["true_label"] == c) & (~df_eval["correct"])].sort_values(by="confidence", ascending=False)
            top20 = pd.concat([c_correct, c_other.iloc[:rem]])
            
        top20_by_emotion[c] = top20
        print(f"  {emo_name:<10}: {len(top20)} samples | Min Conf: {top20['confidence'].min():.3f} | Max Conf: {top20['confidence'].max():.3f}")

    # 6. Generate Grad-CAM for all 140 samples & Build Galleries
    print("\n[5/5] Generating Grad-CAM for all 140 samples and building galleries...")
    master_summary = []
    ...
    for c in range(7):
        ...
        # Save Contact Sheet (20 rows x 3 columns: Original | Heatmap | Overlay)
        ...
        plt.close(fig_grid)
        print(f"  [{emo_name}] Saved Contact Sheet and 4x5 Grid to {emo_dir}")

    # 7. Build HTML interactive gallery
...
"""
  ...
  <div class="details">Dataset Index: {s['dataset_idx']}</div>
    ...
    print(f"\n[SUCCESS] Generated HTML interactive gallery: {html_path.resolve()}")
    print(f"[SUCCESS] Saved manifest CSV: {summary_csv.resolve()}")
    print(f"[SUCCESS] Total candidate images generated: {len(master_summary)} samples (420 individual PNGs)")
