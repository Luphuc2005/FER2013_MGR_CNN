#!/bin/bash
#SBATCH --job-name=FERPLUS_V4_SOTA
...
#SBATCH --mem=64G
#SBATCH --output=/home/ptbao/projects/FER2013_MGR_CNN/logs/FERPLUS_V4_SOTA_%j.out
#SBATCH --error=/home/ptbao/projects/FER2013_MGR_CNN/logs/FERPLUS_V4_SOTA_%j.err
set -euo pipefail

ROOT=/home/ptbao/projects/FER2013_MGR_CNN
cd "$ROOT"
FER_PY="$ROOT/fer2013_env/bin/python"
CONFIG="$ROOT/config_ferplus_v4_minority_direct_fusion_sota.yaml"
OUTPUT_DIR="$ROOT/outputs/papers/ferplus_v4_minority_direct_fusion_sota"
...
echo 'FERPlus V4 SOTA (Target >= 91.5%): Contempt (800) + Disgust (800) + Fear (1000) + Direct 1024-dim Fusion'
echo 'SAM (rho=0.02) + AdamW (6e-6 backbone) + DropPath 0.28 + Gate Floor T_end=1.6 + Semantic Decay (0.10 -> 0.01).'
echo 'Selection via val_accuracy (No-TTA). Target single-model test accuracy >= 91.5%.'

"$FER_PY" -u scripts/smoketest_ferplus_pipeline.py --num-batches 1 --config "$CONFIG"
if [[ "${SMOKE_ONLY:-0}" == "1" ]]; then
    echo 'FERPLUS_V4_SMOKE_ONLY_COMPLETE; training not started.'
    exit 0
fi
...
echo "FERPLUS_V4_TRAIN_COMPLETE: $OUTPUT_DIR"
