...
class HierarchicalCascadeBridge(tf.keras.layers.Layer):
    ...
    def __init__(self, in_channels: int = 512, out_channels: int = 1024, **kwargs):
        super().__init__(**kwargs)
        self.dw_conv = tf.keras.layers.DepthwiseConv2D(kernel_size=3, strides=2, padding="same", name="dw_conv")
        self.pw_conv = tf.keras.layers.Conv2D(out_channels, kernel_size=1, strides=1, padding="valid", name="pw_conv")
