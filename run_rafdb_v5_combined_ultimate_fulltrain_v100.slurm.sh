...
#SBATCH --mem=64G
#SBATCH --output=/home/ptbao/projects/FER2013_MGR_CNN/logs/RAFDB_V5_FULL_%j.out
#SBATCH --error=/home/ptbao/projects/FER2013_MGR_CNN/logs/RAFDB_V5_FULL_%j.err
set -euo pipefail
...
"$FER_PY" -u scripts/check_rafdb_v5_fulltrain.py --config "$CONFIG"
"$FER_PY" -u scripts/smoketest_rafdb_pipeline.py "$CONFIG"

"$FER_PY" -u train.py --config "$CONFIG" --no-auto-increment
echo "Finished: $OUTPUT_DIR (v5 Combined Ultimate Final Full-Train Protocol)."
